...
unsigned char RecBuff_1[REC_BUFF_MAX_LENGTH_1];
unsigned char RecBuffindex_1;
unsigned char RecFlg_1;
unsigned char RecBuff_estLength_1 = REC_BUFF_MAX_LENGTH_1;
...
ISR(USART1_RX_vect){// 인터럽트 루틴에서의 데이터 수신
    RecBuff_1[RecBuffindex_1] = UDR1;
    RecBuffindex_1++;
    if(RecBuffindex_1 > 4){  // 데이터 길이정보가 수신된 경우 패킷 길이 갱신
        RecBuff_estLength_1 = (RecBuff_1[4] + 6); // 패킷길이는 데이터 길이+6
    }
    if(RecBuffindex_1 == RecBuff_estLength_1){          // 수신된 데이터의 순서가 패킷 길이와 같으면 패킷 수신완료
        RecFlg_1 = 1; // 수신 완료 플래그 활성화
    }
}
